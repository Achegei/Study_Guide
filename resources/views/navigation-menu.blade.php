<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                        ...
                        <x-dropdown align="right" width="60">
                            <x-slot name="trigger">
                                <span class="inline-flex rounded-md">
                                    <button type="button" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none focus:bg-gray-50 active:bg-gray-50 transition ease-in-out duration-150">
                                        {{-- ✅ Auth check for current team name --}}
                                        @auth
                            ...
                            <x-slot name="content">
                                <div class="w-60">
                                    ...
                                    {{-- ✅ Auth check for team settings and creation --}}
                                    @auth
                                        ...
                                        @endif
                                    @else
                                        {{-- Fallback for guests if team features are relevant --}}
                                        <div class="block px-4 py-2 text-xs text-gray-400">
                                            {{ __('Please log in to manage teams.') }}
                                        </div>
                                    @endauth
                                </div>
                            </x-slot>
                ...
                <div class="ms-3 relative">
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            {{-- ✅ Auth check for profile photo and user name display --}}
                            @auth
                                ...
                                @endif
                            @else
                                {{-- Fallback for unauthenticated users (e.g., Login/Register buttons) --}}
                                <span class="inline-flex rounded-md">
                                    <button type="button" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none focus:bg-gray-50 active:bg-gray-50 transition ease-in-out duration-150">
                                        {{ __('Account') }}
                                        <svg class="ms-2 -me-0.5 size-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        ...
                        <x-slot name="content">
                            ...
                            {{-- ✅ Auth check for account management links --}}
                            @auth
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            {{-- ✅ Auth check for responsive user info --}}
            @auth
            ...
            @else
                {{-- Fallback for guests in responsive menu --}}
                <div class="flex items-center px-4">
                    <div>
                        <div class="font-medium text-base text-gray-800">{{ __('Guest') }}</div>
                        <div class="font-medium text-sm text-gray-500">{{ __('Please log in or register') }}</div>
                    </div>
                </div>
                <div class="mt-3 space-y-1">
