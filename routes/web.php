...
Route::get('/sitemap.xml', function () {
    // Set your base URL for the sitemap
    $baseUrl = 'https://iqracanadatestprep.ca';

    // Start with the SitemapGenerator (crawler)
    $sitemap = SitemapGenerator::create($baseUrl)
        // Optionally, configure the crawler. For example, to set timeout:
        // ->configureCrawler(function (\Spatie\Crawler\Crawler $crawler) {
        //     $crawler->setParseableMimeTypes(['text/html', 'text/plain']); // Adjust if needed
        //     $crawler->setCrawlQueue(new \Spatie\Crawler\CrawlQueues\RedisCrawlQueue(new \Illuminate\Support\Facades\Redis(), 'default')); // Example for Redis queue
        //     // $crawler->executeJavaScript(); // Only if your links are generated by JS
        // })
        // Use getSitemap() to then add manual URLs and model data
        ->getSitemap();

    // Add static pages manually
    $sitemap->add(Url::create('/'));
    $sitemap->add(Url::create('/purchase'));
    $sitemap->add(Url::create('/buy-now'));
    $sitemap->add(Url::create('/faqs'));
    $sitemap->add(Url::create('/testimonials'));
    $sitemap->add(Url::create('/free-test'));
    $sitemap->add(Url::create('/free-quiz'));
    $sitemap->add(Url::create('/free-driver-quiz'));
    $sitemap->add(Url::create('/canadian-citizenship-prep'));
    $sitemap->add(Url::create('/about'));
    $sitemap->add(Url::create('/terms-of-use'));
    $sitemap->add(Url::create('/policy'));
    $sitemap->add(Url::create('/disclaimer'));
    $sitemap->add(Url::create('/cookie-policy'));
    $sitemap->add(Url::create('/copyright'));
    $sitemap->add(Url::create('/contactus'));

    // Add dynamically generated pages, e.g., blog posts
    // Ensure Post model exists and 'slug' attribute is available
    Post::where('status', 'PUBLISHED')->get()->each(function (Post $post) use ($sitemap) {
        $sitemap->add(
            Url::create("/canadian-citizenship/info/{$post->slug}")
